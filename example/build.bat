...
:: Find ninja -- PATH first, then common off-PATH locations.
set "NINJA_EXE="
for /f "usebackq delims=" %%p in (`where ninja 2^>nul`) do if not defined NINJA_EXE set "NINJA_EXE=%%p"
if not defined NINJA_EXE for %%d in (
    "%ProgramFiles%\ninja-build\ninja.exe"
    "%ProgramFiles(x86)%\ninja-build\ninja.exe"
...
) do if not defined NINJA_EXE if exist %%d set "NINJA_EXE=%%~d"
if not defined NINJA_EXE (
    call :BANNER
    call :ERR "ninja not found.  Add it to PATH, or install via: choco install ninja"
    exit /b 1
)
...
if "%TARGET%"=="" (
    call :ERR "unknown choice -- pick 1-18, A, T, or 0."
    exit /b 1
)
...
:: --- Subroutines --------------------------------------------------------
:CONFIGURE
:: Configure once -- cmake re-runs only when CMakeLists.txt changes.
if not exist "build\CMakeCache.txt" (
    echo %C_DIM%[cmake] configuring...%C_RESET%
